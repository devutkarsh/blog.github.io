...
- `/validate` - This is to also to Authenticate token but most importantly Authorize and validate whether a requested endpoint or URL with a valid token is allowed access to the user based on it's user-type. This is controlled using the [Open Policy Agent](https://www.openpolicyagent.org/) or say OPA.
